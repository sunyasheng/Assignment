...
int Mesh::CountConnectedComponents()
{
    ...
    for(int i=0;i<vList.size();i++)vList[i]->dfsvisit = 0;
    for(int i=0;i<heList.size();i++){
        Vector3d normal_f = heList[i]->LeftFace()->Normal_f();
        if(heList[i]->Start()->dfsvisit==1||normal_f[1]>=0)continue;
        DFSVisit(heList[i]->Start());
        no_component ++;
    }
...
void Mesh::FindCompoBoundary(){
    for(int i=0;i<heList.size();i++)heList[i]->isCompoBoundary = 0;
    for(int i=0;i<heList.size();i++){
        if(heList[i]->IsBoundary()==true)continue;
        Vector3d normal_f1 = heList[i]->LeftFace()->Normal_f();
        if(heList[i]->Twin()->IsBoundary()==true)continue;
        Vector3d normal_f2 = heList[i]->Twin()->LeftFace()->Normal_f();
        if(normal_f2[1]>=0&&normal_f1[1]<0)heList[i]->isCompoBoundary = 1;
    }
}
void Mesh::DFSVisit(Vertex * v){
    if(v->dfsvisit ==1)return ;
    v->dfsvisit = 1;
    HEdge* he = v->HalfEdge(); HEdge* curr = he;
    while(1){
        Vector3d normal_f = curr->LeftFace()->Normal_f();
        if(normal_f[1]<0)DFSVisit(curr->End());
        curr = curr->Twin()->Next();
        while(curr->IsBoundary()==true)curr = curr->Twin()->Next();
        if(curr == he)break;
    }
}
void Mesh::ComputeSupportArea(){
    double res = 0.0;
    for(int i=0;i<fList.size();i++){
        ...
        if(f->Normal_f()[1]<0){
            double side[3];
            side[0] = sqrt((pos2-pos1).Dot(pos2-pos1));
            side[1] = sqrt((pos3-pos1).Dot(pos3-pos1));
            side[2] = sqrt((pos2-pos3).Dot(pos2-pos3));
            double p = (side[0]+side[1]+side[2])/2;
            res += sqrt(p*(p-side[0])*(p-side[1])*(p-side[2]));
        }
    }
    cout<<"support areas of the models is "<<res<<endl;
}

void Mesh::ComputeNormal(Face*f) {
    const Vector3d & pos1 = f->HalfEdge()->Start()->Position();
    const Vector3d & pos2 = f->HalfEdge()->End()->Position();
    const Vector3d & pos3 = f->HalfEdge()->Next()->End()->Position();
