...
void MouseFunc(int button, int state, int x, int y) {
    ...
    if (currentMode == Moving && state == GLUT_DOWN) {
        ...
        for(int i=0;i<0;i++){
            BufferSmoothing* buffersmoother = new BufferSmoothing(&mesh);
            buffersmoother->Deform();
        }
...
int x1 = downX, x2 = lastX, y1 = winHeight - downY, y2 = winHeight - lastY; if (x2 < x1) { int tmp = x1; x1 = x2; x2 = tmp; } if (y2 < y1) { int tmp = y1; y1 = y2; y2 = tmp; } GLProjector projector; VertexList vList = mesh.Vertices(); if (shiftDown)  { for (size_t i = 0; i < vList.size(); i++) { Eigen::Vector3d v = projector.Project(vList[i]->Position()); if ((v(0) >= 0 && v(0) < winWidth && v(1) >= 0 && v(1) < winHeight) && (v(0) > x1 && v(0) < x2 && v(1) > y1 && v(1) < y2)) vList[i]->SetFlag(1); } } else { for (size_t i = 0; i < vList.size(); i++) { Eigen::Vector3d v = projector.Project(vList[i]->Position()); if ((v(0) >= 0 && v(0) < winWidth && v(1) >= 0 && v(1) < winHeight) && (v(0) > x1 && v(0) < x2 && v(1) > y1 && v(1) < y2)) vList[i]->SetFlag(1); else vList[i]->SetFlag(0); } }
}
